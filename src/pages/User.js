...
export default class User extends Component {
  ...
  _getUserData() {
    axios
      .get(
        `/lol/summoner/v4/summoners/by-name/${this.state.id}?api_key=${API_KEY}`
      )
      .then((User) => {
        axios
          .get(
            `/lol/league/v4/entries/by-summoner/${User.data.id}?api_key=${API_KEY}`
          )
          .then((UserData) => {
            axios
              .get(
                `/lol/champion-mastery/v4/champion-masteries/by-summoner/${User.data.id}?api_key=${API_KEY}`
              )
              .then((UserChamp) => {
                axios
                  .get(
                    `/lol/match/v4/matchlists/by-account/${User.data.accountId}?api_key=${API_KEY}`
                  )
                  .then((UserMatch) => {
                    axios
                      .get(
                        `/lol/match/v4/matches/${UserMatch.data.matches[0].gameId}?api_key=${API_KEY}`
                      )
                      .then((MatchData) => {
                        this.setState({
